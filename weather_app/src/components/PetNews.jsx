...
export default function PetNews() {
  ...
  const fetchNewsPage = useCallback(async (q, page = 1) => {
    ...
    const res = await newsAxios.get("/everything", {
      params: { q, language: "en", sortBy: "publishedAt", pageSize: NEWS_PAGE_SIZE, page },
    });
  ...
  const ensurePool = useCallback(
    async (n = 8) => {
      ...
      if (cacheRef.current.pool.length - cacheRef.current.poolIndex >= n) return;

     
      let page = 1;
      let attempts = 0;
      while (cacheRef.current.pool.length - cacheRef.current.poolIndex < n && attempts < 8) {
        const fetchPromises = queries.map((q) => fetchNewsPage(q, page).catch(() => []));
        const results = await Promise.all(fetchPromises);
        const flat = results.flat();
       
        const titleMatches = flat.filter((it) => titleHasPetKeyword(it));
        const fallbackMatches = flat.filter((it) => !titleHasPetKeyword(it) && textHasPetKeyword(it));
        const combined = dedupeByUrl([...cacheRef.current.pool, ...titleMatches, ...fallbackMatches]);
    ...
    async (forceFetch = false) => {
      ...
      try {
        ...
        if (cacheRef.current.pool.length - cacheRef.current.poolIndex < 4) {
          ...
          try {
            const res = await newsAxios.get("/everything", {
              params: { q: "pets OR animals", language: "en", sortBy: "publishedAt", pageSize: 40 },
            });
            ...
            cacheRef.current.pool = dedupeByUrl([...cacheRef.current.pool, ...extra.filter((it) => textHasPetKeyword(it))]);
          } catch {
        ...
        cacheRef.current.poolIndex = idx % Math.max(1, cacheRef.current.pool.length);
        ...
        console.error("PetNews buildBatch failed:", err?.response?.data || err?.message || err);
  ...
  return (
    <Section role="region" aria-label="Pet news">
      <Container>
        <Header>
        ...
        {error ? (
          ...
          <>
            <Row>
              {visible.map((a, i) => (
                <Item key={i} href={a.url} target="_blank" rel="noopener noreferrer" aria-label={a.title}>
                  <ImgWrap>
                    {a.image ? <img src={a.image} alt={a.title} /> : <NoImage>🐾</NoImage>}
                  </ImgWrap>
                  <Info>
            ...
            <SeeMoreRow>
              <SeeMoreBtn onClick={handleSeeMore} disabled={loading} aria-disabled={loading}>
...
const Header = styled.div`
  ...
  margin-bottom:12px;
`;
const Title = styled.h3`margin:0; font-size:20px; font-weight:900; color:#07253a;`;
...
const Row = styled.div`
  ...
  gap:12px;
  align-items:stretch;
  @media (max-width: 1200px) { gap:10px; }
`;
...
const Item = styled.a`
  ...
  &:hover { transform: translateY(-6px); box-shadow: 0 12px 30px rgba(11,59,90,0.08); }
`;
...
const NoImage = styled.div`
  ...
  justify-content:center;
  font-size:36px;
  background: linear-gradient(135deg, rgba(255,122,0,0.06), rgba(7,37,58,0.04));
`;
...
const Meta = styled.div`
  ...
  font-size:12px;
`;
const Source = styled.div`font-weight:700;`;
...
const SeeMoreBtn = styled.button`
  ...
  &:disabled, &[aria-disabled="true"] { opacity: 0.45; cursor: default; transform: none; }
`;

const Error = styled.div`padding:12px; color:#9b1c1c;`;
